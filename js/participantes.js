...
const obtenerParticipantes = async () => {
  try {
    ...
    const respuesta_inscripciones = await axios.get("http://localhost:3000/inscripciones")
    ...
    datos.forEach((participante) => {
      ...
      const inscripcion = respuesta_inscripciones.data.find(i => i.participanteId == participante.id)
      // Si tiene inscripcion muestra el nombre del evento, sino mostrara un guion
      let participante_evento = "-"
      if (inscripcion) {
        const evento = respuesta_eventos.data.find(e => e.id == inscripcion.eventoId)
        if (evento) {
...
async function crear_participante() {
  ...
  try {
    ...
    const modal = bootstrap.Modal.getInstance(document.getElementById("modal_participante"))
    modal.hide()
